...
def format_results(results):
    ...
    text += "\n"
    text += f"{'Buchstaben/Wort (W)':<40}: {results['W']:<10.4f} 95% CI [{results['ci_w'][0]:.4f}, {results['ci_w'][1]:.4f}]\n"
    text += f"{'Buchstaben/Satz (SZ)':<40}: {results['SZ']:<10.4f} 95% CI [{results['ci_sz'][0]:.4f}, {results['ci_sz'][1]:.4f}]\n"
    text += f"{'Kommas/Punkt (K)':<40}: {results['K']:<10.4f} 95% CI [{results['ci_k'][0]:.4f}, {results['ci_k'][1]:.4f}]\n"
    text += f"{'P Kleinwörter':<40}: {results['P']:<10.4f} 95% CI [{100*results['ci_p'][0]:.2f}%, {100*results['ci_p'][1]:.2f}%]\n"
    text += f"{'Q Großwörter':<40}: {results['Q']:<10.4f} 95% CI [{100*results['ci_q'][0]:.2f}%, {100*results['ci_q'][1]:.2f}%]\n"
    text += "\n"
    text += f"{'Implizites Level L1':<40}: {results['L1']:<6.2f}\n"
    text += f"{'Implizites Level L2':<40}: {results['L2']:<6.2f}\n"
    text += f"{'Implizites Level L3':<40}: {results['L3']:<6.2f}\n"
    text += f"{'Implizites Level L4':<40}: {results['L4']:<6.2f}\n"
    text += f"{'Implizites Level L5':<40}: {results['L5']:<6.2f}\n"
    text += f"\n{'Durchschnittliches Sprachniveau (LT)':<40}: {results['LT']:<6.2f}\n"
    text += "```"
    return text
